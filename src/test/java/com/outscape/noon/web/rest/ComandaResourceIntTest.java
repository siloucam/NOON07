...
public class ComandaResourceIntTest {
    ...
    @Autowired
    private ComandaSearchRepository comandaSearchRepository;
    ...
    public void initTest() {
        comandaSearchRepository.deleteAll();
    ...
    public void createComanda() throws Exception {
        ...
        // Validate the Comanda in Elasticsearch
        Comanda comandaEs = comandaSearchRepository.findOne(testComanda.getId());
        assertThat(comandaEs).isEqualToIgnoringGivenFields(testComanda);
    }
    ...
    public void deleteComanda() throws Exception {
        ...
        boolean comandaExistsInEs = comandaSearchRepository.exists(comanda.getId());
        assertThat(comandaExistsInEs).isFalse();
    ...
    @Test
    @Transactional
    public void searchComanda() throws Exception {
        // Initialize the database
        comandaService.save(comanda);

        // Search the comanda
        restComandaMockMvc.perform(get("/api/_search/comandas?query=id:" + comanda.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(comanda.getId().intValue())))
            .andExpect(jsonPath("$.[*].numero").value(hasItem(DEFAULT_NUMERO)))
