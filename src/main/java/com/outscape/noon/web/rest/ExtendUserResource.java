...
public class ExtendUserResource {
    ...
    /**
     * SEARCH  /_search/extend-users?query=:query : search for the extendUser corresponding
     * to the query.
     *
     * @param query the query of the extendUser search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/extend-users")
    @Timed
    public ResponseEntity<List<ExtendUser>> searchExtendUsers(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ExtendUsers for query {}", query);
        Page<ExtendUser> page = extendUserService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/extend-users");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
